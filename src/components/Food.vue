<template>
  <div class="food-item">
    <div style="">
      ...
      <h6 class="food-title" v-html="msg.productName">{{msg.productName}}</h6>
      <h3 class="sub-title ellipsis">{{msg.subTitle}}</h3>
      <div class="food-price pr">
        <div class="ds pa">
          <a @click="openProduct(msg.productId)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      foodsDetails (id) {
        this.$router.push({path: 'foodsDetails/productId=' + id})
      },
      openProduct (id) {
        window.open('//' + window.location.host + '/#/foodsDetails?productId=' + id)
      },
